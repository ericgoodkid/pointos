$(document).ready(function () {
    ...
    const oManage = function() {
        ...
        const createItem = async function() {
            const oItem = getForm();
            const oResult = await axios.post('/api/admin/additionalExpenseType', {
                oItem
        ...
        const updateItem = async function() {
            const oItem = getForm();
            const oResult = await axios.put('/api/admin/additionalExpenseType', {
                oItem
            }).then(oResponse => oResponse.data);
        ...
        const getItem = async function(sCode) {
            return await axios.get(`/api/admin/additionalExpenseType/${sCode}`)
            .then(oResponse => oResponse.data);
        }
        ...
        const deleteItem = async function(oEvent) {
            ...
            const oResponse = await axios.delete('/api/admin/additionalExpenseType', {
                data : { 
                    oItem : {
                        code : sCode
                    } 
        ...
        const initTable = function() {
            return getConstant.oTable.DataTable({
                'ajax': {
                    'url'  : '/api/admin/additionalExpenseType',
                    'type' : 'get'
                },
                order: [[1, 'desc']]
            });
